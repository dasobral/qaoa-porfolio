...
class MarketDataLoader:
    ...
    def __init__(self):
        ...
        self.rate_limit_enabled = config.get(
            "performance.conservative_rate_limiting", True
        )
    ...
    def _show_free_tier_info(self) -> None:
        ...
        logger.info(
            f"⏱️ Rate limiting: {'enabled' if self.rate_limit_enabled else 'disabled'}"
        )
    ...
    async def load_portfolio_data(
        ...
        try:
            ...
            if failed_symbols:
                logger.warning(
                    f"Failed to load data for symbols: {', '.join(failed_symbols)}"
                )
    ...
    def _load_single_asset_sync(
        ...
        try:
            data = self._load_from_yfinance(
                symbol, start_date, end_date, include_volume
            )
        ...
        try:
            ...
            if not data.empty:
                ...
                if index_tz is not None:
                    start_bound = (
                        start_bound.tz_localize(index_tz)
                        if start_bound.tzinfo is None
                        else start_bound.tz_convert(index_tz)
                    )
                    end_bound = (
                        end_bound.tz_localize(index_tz)
                        if end_bound.tzinfo is None
                        else end_bound.tz_convert(index_tz)
                    )
                elif start_bound.tzinfo is not None or end_bound.tzinfo is not None:
                    start_bound = start_bound.tz_localize(None)
                    end_bound = end_bound.tz_localize(None)

                data = data.loc[start_bound:end_bound]

            if data.empty:
                raise MarketDataError(
                    f"No data returned for {symbol} in the specified date range."
                )
    ...
    def _validate_basic_asset_data(self, data: pd.DataFrame, symbol: str) -> None:
        ...
        price_cols = [
            col for col in expected_cols + ["adj_close"] if col in data.columns
        ]
    ...
    def _combine_portfolio_data(
        self, portfolio_data: Dict[str, pd.DataFrame], include_volume: bool
    ) -> pd.DataFrame:
        ...
        if cache_file.exists():
            try:
                ...
                if file_age_days > self.cache_duration_days:
                    logger.debug(
                        f"Cache expired for {symbol}, age: {file_age_days} days"
                    )
        ...
        try:
            if data.empty:
                return {
                    ...
                    "data_points": 0,
                    "data_quality": {"missing_values": 0, "missing_percentage": 0.0},
                    "symbols_count": 0,
