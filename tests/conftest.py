...
def test_config(temp_config_dir):
    ...
    config_data = {
        ...
        "logging": {"show_free_tier_tips": False},  # Reduce noise in tests
    ...
    def _create_mock_data(symbols: List[str], days: int = 30) -> pd.DataFrame:
        ...
        for symbol in symbols:
            ...
            for col in ["open", "high", "low", "close", "volume"]:
                if col == "volume":
                    data[(symbol, col)] = np.random.randint(
                        1000000, 10000000, len(date_range)
                    )
                elif col == "high":
                    data[(symbol, col)] = [
                        p * np.random.uniform(1.00, 1.05) for p in prices
                    ]
                elif col == "low":
                    data[(symbol, col)] = [
                        p * np.random.uniform(0.95, 1.00) for p in prices
                    ]
                elif col == "open":
                    data[(symbol, col)] = [
                        p * np.random.uniform(0.98, 1.02) for p in prices
                    ]
                else:  # close
...
class TestDataValidator:
    ...
    def validate_dataframe_structure(
        df: pd.DataFrame, expected_symbols: List[str]
    ) -> bool:
        ...
        try:
            ...
            if expected_exception and isinstance(e, expected_exception):
                ...
                raise AssertionError(
                    f"Unexpected exception in async test: {type(e).__name__}: {e}"
                ) from e
